    ...
    <div className="grid lg:grid-cols-2 gap-10">
        ...
        <div className="bg-[#0d1526] rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-[#1e2d4a]">
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-3">
              <label className="text-sm font-semibold text-slate-300">Investment Corpus</label>
              <span className="text-sm font-bold text-blue-400 bg-[#162038] px-3 py-1 rounded-lg">{formatCurrency(corpus)}</span>
            </div>
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-3">
              <label className="text-sm font-semibold text-slate-300">Monthly Withdrawal</label>
              <span className="text-sm font-bold text-blue-400 bg-[#162038] px-3 py-1 rounded-lg">{formatCurrency(withdrawal)}</span>
            </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <label className="text-sm font-semibold text-slate-300">Expected Return (% p.a.)</label>
        ...
        <div className="bg-[#162038]/60 rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-[#1e2d4a]">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-semibold ${(withdrawal * 12 / corpus) * 100 <= 4 ? "text-emerald-400" : "text-amber-600 text-amber-400"}`}>
      ...
      <div className="space-y-6">
        <div className={`rounded-2xl sm:rounded-3xl p-4 sm:p-8 border-2 ${isCorpusSustainable
          ...
          <p className={`text-4xl font-black mb-2 ${isCorpusSustainable ? "text-emerald-700 text-emerald-400" : "text-amber-700 text-amber-400"}`}>
          ...
          <div className="space-y-4 mb-6">
            {[
              ...
              <div key={r.label} className="flex items-center justify-between py-3 border-b border-[#1e2d4a] last:border-0">
                ...
                <span className={`text-sm font-bold ${r.accent ? "text-blue-400" : "text-white"}`}>{r.value}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {result.schedule.slice(0, 24).map((row) => (
                  <tr key={row.month} className="border-t border-[#1e2d4a] hover:bg-[#162038]">
                    <td className="px-6 py-3 font-medium text-[slate-300]">Month {row.month}</td>
                    <td className="px-6 py-3 text-blue-400">{formatCurrency(row.withdrawal)}</td>
