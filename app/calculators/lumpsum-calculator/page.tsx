  ...
  return (
    <div className="grid lg:grid-cols-5 gap-8">
      <div className="lg:col-span-2">
        <div className="bg-white bg-[#162038] rounded-2xl p-6 border border-[#1e2d4a] shadow-sm">
            ...
            <div key={inp.label} className="mb-6">
              ...
              {inp.isAmt && (
                <input type="number" value={inp.value} onChange={(e) => inp.set(Number(e.target.value))}
                  className="mt-2 w-full px-3 py-2 rounded-lg border border-[#1e2d4a] bg-white dark:bg-slate-900 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              )}
            </div>
          ))}
        </div>
      ...
      <div className="lg:col-span-3 space-y-6">
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={c.label} className={`p-4 rounded-2xl border ${c.em === "amber" ? "bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800" : c.em === "emerald" ? "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800" : "bg-white bg-[#162038] border-[#1e2d4a]"}`}>
              <div className={`text-xl font-extrabold mb-1 ${c.em === "amber" ? "text-amber-700 text-amber-400" : c.em === "emerald" ? "text-emerald-700 text-emerald-400" : "text-white"}`}>{c.value}</div>
              <div className="text-xs text-slate-500">{c.label}</div>
            </div>
          ))}
        </div>
        <div className="bg-white bg-[#162038] rounded-2xl p-6 border border-[#1e2d4a]">
