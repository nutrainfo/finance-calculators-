    ...
    <div className="grid lg:grid-cols-2 gap-10">
      <div className="space-y-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-slate-200 dark:border-slate-800">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-8">SWP Parameters</h2>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/60 rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-slate-200 dark:border-slate-700">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">Annual withdrawal</span>
              <span className="font-semibold text-slate-900 dark:text-white">{formatCurrency(withdrawal * 12)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500 dark:text-slate-400">Withdrawal rate</span>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          <div className="px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-slate-800">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">First 24 Months</h3>
          </div>
              ...
              <tbody>
                {result.schedule.slice(0, 24).map((row) => (
                  <tr key={row.month} className="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/40">
                    <td className="px-6 py-3 font-medium text-slate-700 dark:text-slate-300">Month {row.month}</td>
                    <td className="px-6 py-3 text-blue-600 dark:text-blue-400">{formatCurrency(row.withdrawal)}</td>
                    <td className="px-6 py-3 font-semibold text-slate-900 dark:text-white">{formatCurrency(row.balance)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
