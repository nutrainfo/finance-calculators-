      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#0d1526] rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-[#1e2d4a]">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-4 bg-[#162038] rounded-2xl">
              <p className="text-xs text-slate-400 mb-1">Absolute Return</p>
              <p className="text-xl font-black text-emerald-400">{result.absoluteReturn}%</p>
            </div>
            <div className="p-4 bg-[#162038] rounded-2xl">
              <p className="text-xs text-slate-400 mb-1">CAGR</p>
              <p className="text-xl font-black text-violet-400">{result.cagr}%</p>
            </div>
          </div>
        </div>
              ...
              <tbody>
                {result.yearlyBreakdown.map((row) => (
                  <tr key={row.year} className="border-t border-[#1e2d4a] hover:bg-[#162038] transition-colors">
                    ...
                    <td className="px-6 py-4 text-right text-emerald-400 font-medium">{formatCurrency(row.returns)}</td>
                    <td className="px-8 py-4 text-right font-bold text-blue-400">{formatCurrency(row.value)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
