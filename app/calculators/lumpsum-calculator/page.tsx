    ...
    <div className="grid lg:grid-cols-5 gap-8">
      <div className="lg:col-span-2">
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-6">Investment Details</h2>
          {[
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={c.label} className={`p-4 rounded-2xl border ${c.em === "amber" ? "bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800" : c.em === "emerald" ? "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800" : "bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700"}`}>
              <div className={`text-xl font-extrabold mb-1 ${c.em === "amber" ? "text-amber-700 dark:text-amber-400" : c.em === "emerald" ? "text-emerald-700 dark:text-emerald-400" : "text-slate-900 dark:text-white"}`}>{c.value}</div>
              <div className="text-xs text-slate-500">{c.label}</div>
            </div>
          ))}
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700">
          <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Growth Chart</h3>
          <GrowthAreaChart data={result.yearlyBreakdown} dataKey="value" investedKey={undefined} />
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700">
          <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Breakdown</h3>
